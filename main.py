...
class Menu:
    ...
    def get_OCLC_retained_holdings(self):
        ...
        for (i, chunk) in chunked_oclc_numbers:
            ...
            oclc_df = pd.DataFrame(columns=['oclcNumber','title', 'allSymbols', 'EAST Symbols', 'Non-UM holdings count', 'EAST holdings count', 'UM held', 'allNames', 'notes'], index=range(0,len(chunk.index)), data=[]*100)
...
async def processResponse(response, params):
    if response.status != 200:
        ...
        data = {'oclcNumber': params["oclcNumber"], 'title': '', 'allSymbols': '', 'EAST Symbols': '', 'Non-UM holdings count': '',  'EAST holdings count': '', 'UM held': '', 'allNames': '', 'notes': f'Error {response.status}'}
    else:
        data = await response.json()
        brief_records = data.get('briefRecords', [])
        if not brief_records:
            data = {'oclcNumber': params['oclcNumber'], 'title': '', 'allSymbols': '', 'EAST Symbols': '', 'Non-UM holdings count': '', 'EAST holdings count': '', 'UM held': '', 'allNames': '', 'notes': 'No holdings'}
        else:
            title = brief_records[0].get('title', '')
            symbols = []
            names = []
            eastSymbolsFile = open(os.getenv('EAST_SYMBOLS'), 'r')
            eastSymbols = [s.strip() for s in eastSymbolsFile.readlines()]
            eastSymbolsFile.close()
            for br in brief_records:
                ...
                data = {'oclcNumber': params['oclcNumber'], 'title': title, 'allSymbols': '|'.join(nonNoneSymbols), 'EAST Symbols': '|'.join(responseEastSymbols), 'Non-UM holdings count': str(len(nonUmassSymbols)), 'EAST holdings count': eastSymbolCount, 'UM held': str(nonUmassSymbols!=nonNoneSymbols), 'allNames': ' | '.join(filter(None, names)), 'notes': ''}
    return data
